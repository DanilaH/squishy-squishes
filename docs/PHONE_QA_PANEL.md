...
The panel must edit the same persisted `SaveStateV2` used by the game. It is not a second progression system and must not require DEV mode.

## Required controls

- set Lab Rank directly to any currently defined rank;
- adjust total Lab XP in small/large steps;
...
The panel may keep only its open/closed UI preference in `sessionStorage`; game/progression data must remain exclusively in the canonical save repository.

## State rules

- XP is clamped to a non-negative integer;
- completed IDs must come only from the canonical recipe registry;
...
- `totalCrafts` must never become lower than the number of completed recipes;
- settings such as mute are untouched.

## UI rules

- add one `QA` launcher beside the existing Metrics/Mesh/Mute controls;
- use a compact scrollable overlay suitable for phone screens;
- do not redesign Collection or the main selector;
- keep controls clearly labelled as QA-only;
- preserve panel-open state across the automatic reload so repeated edits are practical.

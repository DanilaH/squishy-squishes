// Temporary draft-only transport: reconstruct at most one omitted base64 character
// per segment against a precomputed source SHA-256. Never guess a code patch.
...
for (let i = 0; i < chunks.length; i++) {
  let data = chunks[i].trim();
  if (hash(data) !== expected[i] && data.length === (i === 7 ? 543 : 999)) {
    let restored = null;
    outer: for (let p = 0; p <= data.length; p++) for (const char of chars) {
      const candidate = data.slice(0,p) + char + data.slice(p);
      if (hash(candidate) === expected[i]) { restored = candidate; break outer; }
    }
    if (restored) { data = restored; console.log(`Verified restoration of one missing byte in segment ${i}`); }
  }
  console.log(`Patch segment ${i}: length=${data.length}; SHA-256=${hash(data)}`);
  if (data.length !== (i === 7 ? 544 : 1000) || hash(data) !== expected[i]) throw new Error(`Corrupt segment ${i}; refusing patch`);
  chunks[i] = data;
}
const patch = brotliDecompressSync(Buffer.from(chunks.join(''), 'base64'));
const actual = hash(patch);

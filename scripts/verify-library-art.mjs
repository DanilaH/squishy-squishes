...
if (!Array.isArray(manifest.active) || manifest.active.length !== expected.size) {
  throw new Error('Expected exactly six active original Library assets');
}
for (const item of manifest.active) {
  ...
  if (item.file !== `src/experiments/phaser/library-assets/${item.name}.webp`) {
  ...
  if (!runtime.includes(`./library-assets/${item.name}.webp`)) {
...
console.log('PASS: all six Hall assets trace to the owner Library commit and match active exports');

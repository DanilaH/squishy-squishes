...
test('desktop: actual Hall and eight 512px candidates share toy and visible size', async ({ page }, testInfo) => {
  const errors: string[] = [];
  ...
  await expect(lab.locator('article canvas')).toHaveCount(9);
  ...
  expect(dimensions.map(entry => entry.size)).toEqual(Array.from({ length: 9 }, () => [512, 512]));
  expect(dimensions.map(entry => entry.profile)).toEqual([
    'studio-shader-512', 'flat-control-512', 'sdf-relief-512', 'pseudo-extruded-512',
    'inflated-mesh-512', 'studio-textured-mesh-512', 'sdf-field-mesh-512',
    'sdf-field-flat-mesh-512', 'contour-mesh-512',
  ]);
  ...
  expect((await inspect())[8]?.size).toEqual([512, 512]);
...
test('phone: nine-way comparison closes without touching saved Hall', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/phaser/?volume-probe=1');
  const lab = page.locator('#library-volume-probe');
  await expect(lab.locator('article canvas')).toHaveCount(9);
  await lab.locator('[data-shape="heart"]').click();
  for (const variant of ['current', 'hires', 'relief', 'extruded', 'mesh', 'mesh-studio', 'mesh-field', 'mesh-field-flat', 'mesh-contour'] as const) {

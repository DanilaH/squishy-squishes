...
test('desktop: current Hall and six 512px candidates keep identical source and visible size', async ({ page }, testInfo) => {
  const errors: string[] = [];
  ...
  await expect(lab.locator('article canvas')).toHaveCount(7);
  ...
  expect(dimensions.map(entry => entry.size)).toEqual(Array.from({ length: 7 }, () => [512, 512]));
...
test('phone: seven-way comparison closes without touching saved Hall', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/phaser/?volume-probe=1');
  const lab = page.locator('#library-volume-probe');
  await expect(lab.locator('article canvas')).toHaveCount(7);
  await lab.locator('[data-shape="heart"]').click();
  for (const variant of ['current', 'hires', 'relief', 'extruded', 'mesh', 'mesh-studio', 'mesh-field'] as const) {

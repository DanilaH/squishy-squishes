...
test('desktop: actual 512px Hall and three 512px candidates share saved art and display size', async ({ page }, testInfo) => {
  const errors: string[] = [];
  ...
  expect(dimensions.map(entry => entry.size)).toEqual([[512, 512], [512, 512], [512, 512], [512, 512]]);
  expect(dimensions.map(entry => entry.profile)).toEqual(['studio-shader-512', 'flat-control-512', 'sdf-relief-512', 'pseudo-extruded-512']);
...
test('phone: honest four-way comparison can close without touching saved Hall', async ({ page }, testInfo) => {

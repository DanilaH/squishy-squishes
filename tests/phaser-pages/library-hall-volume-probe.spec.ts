...
test('desktop: real 512px Hall and five 512px candidates share saved art and display size', async ({ page }, testInfo) => {
  const errors: string[] = [];
  ...
  await expect(lab.locator('article canvas')).toHaveCount(6);
  ...
  for (const shape of ['soft-square', 'heart', 'paw'] as const) {
    ...
    await expect(lab.locator('[data-variant="mesh"] canvas')).toHaveAttribute('data-volume-renderer', 'inflated-mesh-512');
    await expect(lab.locator('[data-variant="mesh-studio"] canvas')).toHaveAttribute('data-volume-renderer', 'studio-textured-mesh-512');
  ...
  expect((await inspect())[5]?.size).toEqual([512, 512]);
...
test('phone: six-way comparison closes without touching saved Hall', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/phaser/?volume-probe=1');
  const lab = page.locator('#library-volume-probe');
  await expect(lab.locator('article canvas')).toHaveCount(6);
  await lab.locator('[data-shape="heart"]').click();
  for (const variant of ['current', 'hires', 'relief', 'extruded', 'mesh', 'mesh-studio'] as const) {

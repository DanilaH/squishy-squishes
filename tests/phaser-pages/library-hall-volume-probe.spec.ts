...
test('desktop: real 512px Hall and four 512px candidates share saved art and display size', async ({ page }, testInfo) => {
  const errors: string[] = [];
  ...
  expect(dimensions.map(entry => entry.size)).toEqual([[512, 512], [512, 512], [512, 512], [512, 512], [512, 512]]);
  expect(dimensions.map(entry => entry.profile)).toEqual([
    'studio-shader-512', 'flat-control-512', 'sdf-relief-512', 'pseudo-extruded-512', 'inflated-mesh-512',
  ]);
...
test('phone: five-way comparison closes without touching saved Hall', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/phaser/?volume-probe=1');
  const lab = page.locator('#library-volume-probe');
  await expect(lab.locator('article canvas')).toHaveCount(5);
  await lab.locator('[data-shape="heart"]').click();
  for (const variant of ['current', 'hires', 'relief', 'extruded', 'mesh'] as const) {
